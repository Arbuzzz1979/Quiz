  ...
  useEffect(() => {
    ...
    const unsubscribe = auth.onAuthStateChanged((user) => {
      ...
      user &&
        ...
        .onSnapshot((doc) => setUserProfile(doc.data()));

      // Get List of Current User Quiz History
      user &&
        db.collection("users").doc(auth.currentUser.uid).collection("quiz")
          .orderBy("sort", "desc")
          .onSnapshot((querySnapshot) => setQuizHistory(_init_UserQuizHistory(querySnapshot)));

      // Get Current Users Avatar Url
      user && 
       storage.ref().listAll()
        .then((result) =>{
          result.items.forEach((imageRef)=>{
            if (imageRef.name===auth.currentUser.uid) {
              imageRef
                .getDownloadURL()
                .then((url)=> setUserAvatar(url))
            }
          })
        })
        // storage.ref().child(auth.currentUser.uid).getDownloadURL()
        //   .then((url) => {
        //     setUserAvatar(url)
        //   })
      ...
      // // Get Top Scores Users
      // db.collection("users")
      //   .orderBy("totalScore", "desc").limit(2)
      //   .onSnapshot((querySnapshot) => setTopScore(_init_TopScoresArray(querySnapshot)));
